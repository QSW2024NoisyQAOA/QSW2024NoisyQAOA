...
def compute_approximation_ratio(samples: Samples, problem: Ising, max_size: float) -> float:
    expected_cut_size = 0
    for bitstring, probability in samples:
...
class NoiseParameters:
    def __init__(self, t1: float, t2: float, gate_errors: dict[str, float], gate_times: dict[str, float], prep0_meas1: float, prep1_meas0: float):
        self.t1 = t1
        self.t2 = t2
...
def compose_parametric_channels(channels: list[Callable[[float], QuantumChannelKraus]], gate_time: float, n_qubits: int) -> QuantumChannelKraus:
...
def create_parametric_channel_by_fidelity(
    channels: list[Callable[[float], QuantumChannelKraus]], target_fidelity: float, n_qubits: int, start=50, acc=0.00001
) -> QuantumChannelKraus:
    current = start
    while True:
...
def create_wsqaoa_ansatz(
    problem: Ising, n_layers: int, solution: list[int], epsilon: float, wsinitqaoa=False, custom_gate=True
) -> Job:
...
def perform_qaoa(ansatz: Job, qpu: CommonQPU, use_qlmaas: bool, compute_circuit_fidelity=False) -> Samples:
    minimizer = QlmaasScipyMinimizePlugin if use_qlmaas else ScipyMinimizePlugin  # type: ignore
    stack = (
        minimizer(
            method="COBYLA",
            tol=1e-2,
...
def execute_model(source: Source, problem: Ising, conn, compute_circuit_fidelity: bool) -> Samples: # type: ignore
    ...
    if source.type == "random":
        return generate_uniform_solution(len(problem.magnetic_field_h))
    if source.type == "approximation":
        if isinstance(problem, MaxCut):
            return sample_gw_rounding(perform_maxcut_sdp(problem.graph), n=10000)
        if isinstance(problem, NumberPartitioning):
            greedy_result = partition_greedy(problem.array_of_numbers)
            bitstring = "".join([str(i) for i in greedy_result])
            return [(bitstring, 1)]
        raise ValueError(f"GW rounding mode only supported for problem {problem}")
...
def execute_quantum_model(
    ...
    if algorithm_type == "rqaoa":
        ...
        def rqaoa_perform_qaoa(problem):
            ...
            return perform_qaoa(ansatz, qpu, use_qlmaas)

        return rqaoa(problem, rqaoa_perform_qaoa, algorithm.n_samples)
    ...
    transpile(ansatz.circuit)
    return perform_qaoa(ansatz, qpu, use_qlmaas, compute_circuit_fidelity)


def worker(seed: int, params: Params, use_qlmaas: bool, verbose: bool) -> float:
    conn = None
    if use_qlmaas:
    ...
    while True:
        try:
            compute_circuit_fidelity = params.metric == "fidelity"
            problem = gen_problem(params.graphs, seed)
            samples = execute_model(
                params.source, problem, conn, compute_circuit_fidelity
            )
...
def main(param_sets: list[Params], meta: str, verbose: bool, n_threads: int, out: str, qlmaas: bool, raw: bool, done_marker: bool):
    ...
    with pool:
        ...
        for results_by_graph, params in zip(total_results, param_sets):
            ...
            for seed, graph_result in enumerate(results_by_graph):
                graph_result = [r.get() for r in graph_result]
                n_shots = len(graph_result)
                graph_result = [r for r in graph_result if r is not None]
                if len(graph_result) != n_shots:
                    print("not all shots completed")
                avg = sum(graph_result) / len(graph_result)
            ...
            params.value = sum(results) / len(results)
            if raw:
                params.raw = results
...
if __name__ == "__main__":
    ...
    parser.add_argument("--params", "-p", required=True, help="Benchmark parameters (#qubits, algorithm, ...) as JSON5 string (cf. parse_params.py)")
    parser.add_argument("-v", action="store_true", help="Enable verbose output (print result for every algorithm run)")
    parser.add_argument("--meta", "-m", default="{}", help="JSON5 string which is attached to the benchmark results for later filtering")
    parser.add_argument("--n_threads", "-P", type=int, default=1, help="Degree of parallelism for the simulations")
    parser.add_argument("--out", "-o", help="Path to the output file (without *.out file extension)")
    parser.add_argument("--qlmaas", action="store_true", help="Use QLMaaS (experimental)")
    parser.add_argument("--raw", "-r", action="store_true", help="Add results for individual instances to output (not just average)")
    parser.add_argument("--done_marker", action="store_true", help="Copy benchmarks results into *.txt file when they are finished")
    ...
    main(param_sets, args.meta, args.v, args.n_threads, args.out, args.qlmaas, args.raw, args.done_marker)
