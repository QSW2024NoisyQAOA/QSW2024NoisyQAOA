...
for problem in problems:
    for n in ns:
        approxmiation_values = [
            ...
            for p in apply_filter(
                params, f"and and graphs.problem {problem} source.type approximation graphs.size {n}"
            )
        ]
        ...
        random_values = [
            p.value
            for p in apply_filter(
                params, f"and and graphs.problem {problem} source.type random graphs.size {n}"
            )
        ]
    ...
    measurements = {
        (alg, noisy): [None] * len(ns) for alg in algs for noisy in noisys
    }
    for alg in algs:
        for noisy in noisys:
            for n in ns:
    ...
    for alg in algs:
        for noisy in noisys:
            for n in ns: 
                ...
                rows.append((value, problem_name[problem], model_name, alg_name[alg], n))
