            ...
            for noise in noise_levels:
                for time in noise_levels:
                    ...
                    for size in range(5, 11):
                        values_p.extend(data[(problem, algorithm, layer, size, noise, time)])
                        values_p_minus_1.extend(data[(problem, algorithm, layer - 1, size, noise, time)])
                    layer_advantage = [a / b for a, b in zip(values_p, values_p_minus_1)]
                    avg = sum(layer_advantage) / len(layer_advantage)
                    rows.append((problem_name[problem], alg_name[algorithm], layer, noise, time, avg))

headers = ("problem","algorithm","layer","depolarizing_noise","thermal_relaxation_noise","relative_performance")
