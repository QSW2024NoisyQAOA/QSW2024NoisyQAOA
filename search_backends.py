...
class Backend:
    def __init__(self, name: str, conf: dict, props: dict):
        ...
        if self.n_couplings is None or self.n_qubits < 10:
            ...
            self.coupling_density = len(self.couplings) / ((self.n_qubits - 1) * self.n_qubits)
            # sum_coupling_densities = 0
            # for qubit in range(self.n_qubits):
            #     sum_coupling_densities += len(
            #         [b for a, b in self.couplings if a == qubit]
            #     ) / (self.n_qubits - 1)
            # self.coupling_density = sum_coupling_densities / self.n_qubits
...
def main(acc_name: str, inner_backend_acc_name: str, backend_name: str, gate_filter: str, verbose: bool):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--acc", choices=acc_names, required=True, help="Function to combine backend results to single value")
    parser.add_argument("--inner_backend_acc", choices=acc_names, help="Function to combine qubit results of one backend to single value (the same as --acc by default)")
    parser.add_argument("--backend", default=None, help="If set the name of a backend (e.g. almaden), only show results for this backend")
    parser.add_argument("--gate_filter", default=None, help="If set (e.g. to \"u3\"), only show use results for backends supporting this gate")
    parser.add_argument("-v", action="store_true", help="If set, show more results (e.g. ratio of T1 and gate duration)")
